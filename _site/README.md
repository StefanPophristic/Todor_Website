...
Movie pages are generated by a **Jekyll plugin** (`_plugins/`). The default GitHub Pages build **does not run** that plugin, so movie URLs **404** unless you deploy with **GitHub Actions**. See **[DEPLOY.md](DEPLOY.md)** for the exact settings (Pages → Source: **GitHub Actions**) and troubleshooting.
