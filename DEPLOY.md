...
The workflow file is **`.github/workflows/jekyll.yml`**. It runs **`npm ci`**, **`npm run generate:images`** (WebP + JPEG variants for posters, film stills, and About/Contact photos), then **`bundle exec jekyll build`** so **`_plugins/` runs** and movie pages exist in `_site/movies/`.
...
Run **`npm run generate:images`** whenever you add or change poster, **film still**, or About/Contact source images (see **README**). Without it, responsive `<picture>` URLs under `images/.../generated/` may be missing.
